...
async function main() {
  ...
  const job2 = await prisma.job.create({ data: { title: '后端平台工程师', department: '产品研发部', location: '杭州', salaryRange: '28k-42k', description: '建设招聘流程服务、权限与审计能力。', requirements: 'NestJS/Node.js、PostgreSQL、Prisma、领域建模经验。', headcount: 1, status: JobStatus.PAUSED, hiringManagerId: manager.id } });
  ...
  const c3 = await prisma.candidate.create({ data: { name: '赵予安', email: 'yuan.zhao@example.com', phone: '13800000003', source: '内推' } });
  ...
  const r3 = await prisma.resume.create({ data: { candidateId: c3.id, jobId: job2.id, resumeUrl: 'https://files.example.com/resumes/zhaoyuan.pdf', status: ResumeStatus.INTERVIEWING } });
  ...
  // 待审批草稿：经理审批后占 1/3 编制，r1 转入 OFFERED；可继续造两个草稿验证满编自动关闭
  ...
  // job2 编制仅 1：审批后立即占满并自动关闭，拒绝/撤回后仅恢复该岗位
  await prisma.offer.create({ data: { candidateId: c3.id, jobId: job2.id, salary: '36000', startDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30), status: OfferStatus.DRAFT, approverId: manager.id } });
